...
public class Wilderness {
    ...
    public static final Bounds WILDERNESS_BOUNDS = new Bounds(2944, 3522, 3393, 3971, 0);
...
//                    RiskProtection.INSTANCE.send(player);
    ...
    public static void lootingBagDrop(Player pKiller, Position dropPosition) {
        if (dropPosition.inBounds(WILDERNESS_BOUNDS)) {
            if (pKiller == null)
                return;
            if (Random.rollDie(30, 1)) {
                if(pKiller.player.getInventory().hasId(11941))
                    return;
                new GroundItem(11941, 1).owner(pKiller.player).position(dropPosition).spawn();
            }
        }
    }
}