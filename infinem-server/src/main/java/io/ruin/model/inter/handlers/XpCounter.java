    ...
    static {
        InterfaceHandler.register(137, h -> {
            ...
            h.actions[17] = (SlotAction) (player, slot) -> {
                ...
                Config.XP_COUNTER_TRACKER_START.set(player, start);
                Config.XP_COUNTER_TRACKER_GOAL.set(player, goal);
                int activeValue = (goal > 0 && start > 0) ? 2 : (start > 0) ? 1 : 0;
                Config.XP_COUNTER_TRACKER_TYPE_ACTIVE.set(player, activeValue);
            };
            // Disable tracker for skill
            h.actions[21] = (SlotAction) (player, slot) -> {
                Config.XP_COUNTER_TRACKER_GOAL.set(player, 0);
                Config.XP_COUNTER_TRACKER_START.set(player, 0);
                Config.XP_COUNTER_TRACKER_TYPE_ACTIVE.set(player, 0);
            };
            // Enable tracker, middle panel
            h.actions[25] = (SlotAction) (player, slot) -> {
                Config.XP_COUNTER_TRACKER_START.set(player, getCurrentXp(player, Config.XP_COUNTER_TRACKER_STAT_VIEWING.get(player)));
                Config.XP_COUNTER_TRACKER_GOAL.set(player, -1);
            ...
            h.actions[33] = (SlotAction) (player, slot) -> {
                ...
                Config.XP_COUNTER_TRACKER_START.set(player, getCurrentXp(player, statType));
                int goalXp;
                if (onTotalMenu(player)) {
                ...
                Config.XP_COUNTER_TRACKER_GOAL.set(player, goalXp);
            };
            ...
            for (int button : SET_BUTTONS) {
                h.actions[button] = (OptionAction) (player, option) -> {
                    ...
                    Config varp = button == 42 ? Config.XP_COUNTER_TRACKER_GOAL : Config.XP_COUNTER_TRACKER_START;
                    if (message.length() > 1) { // Set to xp or level
                        ...
                        player.integerInput(message, amount -> {
                            ...
                            if (varp == Config.XP_COUNTER_TRACKER_START && xp > finalCurrentXp) {
            ...
            h.actions[46] = (SlotAction) (player, slot) -> {
                StatType statType = statTypeOfCurrentMenu(Config.XP_COUNTER_TRACKER_STAT_VIEWING.get(player));
                long currentXp = (onTotalMenu(player) || statType == null) ? player.getStats().totalXp : (long) player.getStats().get(statType).experience;
                int start = Config.XP_COUNTER_TRACKER_START.get(player);
                int multiplier = onTotalMenu(player) ? 1000 : 1;
                if (start * multiplier > currentXp) {
                ...
                int goal = Config.XP_COUNTER_TRACKER_GOAL.get(player);
