...
public class MotherlodeMine { //why do we have two motherlode mine classes? Remove the one from skilling and add the actions here
    ...
    static {
        ...
        ObjectAction.register(DARK_TUNNEL, "enter", (player, obj) -> {
            if(!player.getStats().check(StatType.Agility, 54, "enter the tunnel")) return;
    ...
    private static void mine(Player player, GameObject obj, boolean upperLevel) {
        ...
        player.startEvent(event -> {
            ...
            while (true) {
                if (player.debug) {
                    double chance = Mining.chance(player.getStats().get(StatType.Mining).currentLevel, Rock.MITHRIL);
                ...
                if(attempts == 0) {
                    ...
                    attempts++;
                } else if (attempts % 2 == 0 && Random.get(100) <= Mining.chance(player.getStats().get(StatType.Mining).currentLevel, Rock.MITHRIL)) {// 300 "difficulty" seems to be a good spot.
