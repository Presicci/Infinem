...
import io.ruin.model.activities.combat.raids.xeric.chamber.impl.CheckpointChamber;
import io.ruin.model.activities.combat.raids.xeric.chamber.impl.OlmChamber;
...
public class ChambersOfXeric {
    ...
    public void generate() {
        ...
        Arrays.stream(ChamberDefinition.values()).filter(def -> def.getType() == ChamberType.PUZZLE).forEach(puzzlePool::add);
        ...
        for (int floorNumber = 0; floorNumber < 2; floorNumber++) {
            ...
            Chamber start = floorNumber == 1 ? ChamberDefinition.START.newChamber() : ChamberDefinition.LOWER_LEVEL_START.newChamber();
            floor[x][y] = start;
            if (floorNumber == 1)
            ...
            while (true) { // add random rooms
                ...
                if (nextMove == null || randomRooms == 6 && rng.nextDouble() > SIXTH_ROOM_CHANCE) { // no moves left, or made 5 rooms and failed roll for 6th
                    if (floorNumber == 1) {
                        upperFinishChamber = (CheckpointChamber) (floor[x][y] = ChamberDefinition.UPPER_FLOOR_FINISH.newChamber());
                    } else {
                        lowerFinishChamber = (CheckpointChamber) (floor[x][y] = ChamberDefinition.LOWER_FLOOR_FINISH.newChamber());
                    }
                    break;
                } else {
    ...
    private Chamber getResourceChamber() {
        return rng.nextDouble() < 0.5 ? ChamberDefinition.FISHING_RESOURCE.newChamber() : ChamberDefinition.HUNTER_RESOURCE.newChamber();
    }

    private Chamber getScavengerChamber() {
        return rng.nextDouble() < 0.5 ? ChamberDefinition.SCAVENGERS.newChamber() : ChamberDefinition.SCAVENGER_RUINS.newChamber();
    }
    ...
    private static String[] getLayout() {
        ...
        for (Chamber room : layout) {
            ...
            if (chamberDefinition.equals(ChamberDefinition.LOWER_LEVEL_START) || chamberDefinition.equals(ChamberDefinition.LOWER_FLOOR_FINISH)) {
                isLowerLevel = !isLowerLevel;
                continue;
            }
            if (chamberDefinition.equals(ChamberDefinition.START) || chamberDefinition.equals(ChamberDefinition.UPPER_FLOOR_FINISH)) {
