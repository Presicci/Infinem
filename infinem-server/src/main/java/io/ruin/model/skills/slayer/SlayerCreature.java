...
public enum SlayerCreature {
    ...
    ABYSSAL_DEMON(42, 85, 85, "abyssal demons", (p, s) -> Config.I_WILDY_MORE_SLAYER.get(p) != 0 || s.getNpcId() != Master.KRYSTILIA.getNpcId()),
    ...
    DUST_DEVILS(49, 65, 70, "dust devils", (p, s) -> Config.I_WILDY_MORE_SLAYER.get(p) != 0 || s.getNpcId() != Master.KRYSTILIA.getNpcId()),
    JELLIES(50, 52, 57, "jellies", (p, s) -> Config.I_WILDY_MORE_SLAYER.get(p) != 0 || s.getNpcId() != Master.KRYSTILIA.getNpcId()),
    ROCKSLUG(51, 20, 20, "rockslugs"),
    NECHRYAEL(52, 80, 85, "nechryael", (p, s) -> Config.I_WILDY_MORE_SLAYER.get(p) != 0 || s.getNpcId() != Master.KRYSTILIA.getNpcId()),
    ...
    public static String taskName(Player player, int uid) {
        ...
        if (bossTask != 0 && uid == 98) {
            StructDefinition struct = StructDefinition.get(EnumDefinition.get(5008).getValuesAsInts().get(bossTask));
            result = (String) struct.getParams().get(1801);
        } else {
