...
public class MarkOfGrace {
    ...
    public static void rollMark(Player player, int levelReq, List<Position> spawns) {
        ...
        if (Random.get() <= chance) {
            ...
            int amount = Random.get(1, 4);
            // Increment base amount by fragment mod
            amount += (int) player.getRelicFragmentManager().getModifierValue(FragmentType.Agility, FragmentModifier.MARKS_OF_GRACE);
            // Finally, double amount if activity is active
            amount *= ActivitySpotlight.isActive(ActivitySpotlight.DOUBLE_MARKS_OF_GRACE) ? 2 : 1;
