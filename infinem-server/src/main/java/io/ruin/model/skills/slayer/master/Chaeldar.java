...
public class Chaeldar {
    ...
    private static void assignTask(Player player) {
        ...
        Config.SLAYER_MASTER.set(player, SlayerMaster.CHAELDAR_ID);
                                                ...
                                                player.dialogue(new PlayerDialogue("Wow, can you teach me?"),
                                                        ...
                                                        new PlayerDialogue("Okay, what's first?").action(() -> {
                                                            ...
                                                            if (task != null) {
                                                                ...
                                                                player.dialogue(new NPCDialogue(CHAELDAR, "We'll start you off hunting " + SlayerCreature.taskName(player, task.getUid()) + ", you'll need to kill " + num + "<br>of them."));
                                                            }
                                                        }));
    ...
    private static void giveTask(Player player) {
        ...
        if (Config.SLAYER_POINTS.get(player) >= 30) {
            player.dialogue(
                    new NPCDialogue(CHAELDAR, "Excellent, you're doing great. Your new task is to kill " + left + " " + SlayerCreature.taskName(player, task.getUid()) + "."),
                    new OptionsDialogue(
                            new Option("Got any tips for me?", new PlayerDialogue("Got any tips for me?"), new NPCDialogue(CHAELDAR, SlayerCreature.getTipFor(task)), new PlayerDialogue("Great, thanks!")),
                            new Option("Great, thanks!", new PlayerDialogue("Okay, great!")),
                            new Option("No thanks. (Reroll task, costs 30 Slayer points)", new NPCDialogue(CHAELDAR, "Very well."), new ActionDialogue(() -> {
    ...
    static {
        NPCAction.register(CHAELDAR, 1, (player, npc) -> handleInteraction(player, npc, 1));
        NPCAction.register(CHAELDAR, 2, (player, npc) -> handleInteraction(player, npc, 2));
        NPCAction.register(CHAELDAR, 3, (player, npc) -> handleInteraction(player, npc, 3));
        NPCAction.register(CHAELDAR, 4, (player, npc) -> handleInteraction(player, npc, 4));
        NPCAction.register(CHAELDAR, 5, (player, npc) -> handleInteraction(player, npc, 5));
    }
}
