...
public class Nieve {
                                                ...
                                                player.dialogue(new PlayerDialogue("Wow, can you teach me?"),
                                                        ...
                                                        new PlayerDialogue("Okay, what's first?").action(() -> {
                                                            ...
                                                            if (task != null) {
                                                                ...
                                                                player.dialogue(new NPCDialogue(NIEVE, "We'll start you off hunting " + SlayerCreature.taskName(player, task.getUid()) + ", you'll need to kill " + num + "<br>of them."));
                                                            }
                                                        }));
    ...
    private static void giveTask(Player player) {
        if (player.getCombat().getLevel() < 85) {
            player.dialogue(new NPCDialogue(NIEVE, "Sorry, but you're not strong enough to be taught by me, you need 85 combat!"));
            return;
        }
        ...
        if (task.equals(SlayerCreature.BOSSES)) {
            player.dialogue(
                    new NPCDialogue(NIEVE, "Your new task is to kill " + SlayerCreature.taskName(player, task.getUid()) +
                        ...
                        public void closed(Player player) {
                            player.integerInput("How many would you like to slay? (3-35)", (i) -> {
                                ...
                                player.dialogue(
                                        new NPCDialogue(NIEVE, "Excellent. You're now assigned to kill " + SlayerCreature.taskName(player, task.getUid()) + " boss " + i + " times."),
                                        new OptionsDialogue(new Option("Got any tips for me?", () -> player.dialogue(
                                                new PlayerDialogue("Got any tips for me?"),
                                                new NPCDialogue(NIEVE, SlayerCreature.getTipFor(task)),
            ...
            if (Config.SLAYER_POINTS.get(player) >= 30) {
                player.dialogue(
                        new NPCDialogue(NIEVE, "Excellent, you're doing great. Your new task is to kill " + left + " " + SlayerCreature.taskName(player, task.getUid()) + "."),
                        new OptionsDialogue(
                                new Option("Got any tips for me?", new PlayerDialogue("Got any tips for me?"), new NPCDialogue(NIEVE, SlayerCreature.getTipFor(task)), new PlayerDialogue("Great, thanks!")),
                                new Option("Great, thanks!", new PlayerDialogue("Okay, great!")),
                                new Option("No thanks. (Reroll task, costs 30 Slayer points)", new NPCDialogue(NIEVE, "Very well."), new ActionDialogue(() -> {
    ...
    static {
        NPCAction.register(NIEVE, 1, (player, npc) -> handleInteraction(player, npc, 1));
        NPCAction.register(NIEVE, 2, (player, npc) -> handleInteraction(player, npc, 2));
        NPCAction.register(NIEVE, 3, (player, npc) -> handleInteraction(player, npc, 3));
        NPCAction.register(NIEVE, 4, (player, npc) -> handleInteraction(player, npc, 4));
        NPCAction.register(NIEVE, 5, (player, npc) -> handleInteraction(player, npc, 5));
    }
}
