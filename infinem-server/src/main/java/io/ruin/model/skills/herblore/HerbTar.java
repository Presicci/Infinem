    ...
    static {
        for (HerbTar herbTar : values()) {
            ...
            ItemItemAction.register(SWAMP_TAR, herbTar.herb, (player, primary, secondary) -> {
                ...
                if (!(player.getInventory().contains(SWAMP_TAR, 15) || player.getInventory().contains(herbTar.herb, 1))) {
                    player.sendMessage("You need 15 swamp tar to mix it with an herb.");
                }
