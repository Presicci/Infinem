...
public class Duradel {
    ...
    private static void assignTask(Player player) {
        SlayerMaster master = SlayerMaster.master(DURADEL);//SlayerMaster.master(DURADEL);
    ...
    private static void giveTask(Player player) {
        if (player.getStats().get(StatType.Slayer).fixedLevel < 50 && player.getCombat().getLevel() < 100) {
            player.dialogue(new NPCDialogue(DURADEL, "Sorry, but you're not strong enough to be taught by me, you need 50 slayer and 100 combat!"));
            return;
        }
        ...
        if (task.equals(SlayerCreature.BOSSES)) {
            player.dialogue(
                    new NPCDialogue(DURADEL, "Your new task is to kill " + SlayerCreature.taskName(player, task.getUid()) +
                            ...
                            player.integerInput("How many would you like to slay? (3-35)", (i) -> {
                                ...
                                player.dialogue(
                                        ...
                                        new OptionsDialogue(new Option("Got any tips for me?", () -> player.dialogue(
                                                ...
                                                new NPCDialogue(DURADEL, SlayerCreature.getTipFor(task)),
            ...
            if (Config.SLAYER_POINTS.get(player) >= 30) {
                player.dialogue(
                        new NPCDialogue(DURADEL, "Excellent, you're doing great. Your new task is to kill " + left + " " + SlayerCreature.taskName(player, task.getUid()) + "."),
                        new OptionsDialogue(
                                new Option("Got any tips for me?", new PlayerDialogue("Got any tips for me?"), new NPCDialogue(DURADEL, SlayerCreature.getTipFor(task)), new PlayerDialogue("Great, thanks!")),
    ...
    static {
        NPCAction.register(DURADEL, 1, (player, npc) -> handleInteraction(player, npc, 1));
        NPCAction.register(DURADEL, 2, (player, npc) -> handleInteraction(player, npc, 2));
        NPCAction.register(DURADEL, 3, (player, npc) -> handleInteraction(player, npc, 3));
        NPCAction.register(DURADEL, 4, (player, npc) -> handleInteraction(player, npc, 4));
        NPCAction.register(DURADEL, 5, (player, npc) -> handleInteraction(player, npc, 5));
    }
}
