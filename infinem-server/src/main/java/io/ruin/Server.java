...
//        KilimAgent.classLoader = (WeavingClassLoader) Thread.currentThread().getContextClassLoader();
         ...
         * Network
         */
        NettyServer nettyServer = NettyServer.start(World.type.getWorldName() + " World (" + World.id + ") Server", World.port, pipeline -> new LoginDecoder(), 5, Boolean.parseBoolean(properties.getProperty("offline_mode")));
